...
export default function OAuth() {
  const { accessToken, expirationTime, refreshToken } = useParams();
  ...
  useEffect(() => {
    // oauth 토큰 전달해주기 //
    if (!accessToken || !expirationTime || !refreshToken) return; // 유효하지 않은 토큰 정보는 처리하지 않습니다.
  ...
  }, [accessToken, expirationTime, refreshToken, navigate, setCookies]); // 의존성 배열에 포함된 변수들이 변경될 때마다 이펙트 실행
